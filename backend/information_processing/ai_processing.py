...
def analyze_article(content):
	...
	messages = [{'role': 'system', 'content': prompt_filter}, {'role': 'user', 'content': content}]
			...
			if analyze_article(content):
				...
				for attempt in range(max_retries):
					try:
						# 调用API获取原始文本
						completion = client.chat.completions.create(
							model='qwen-plus', messages=messages, temperature=0.1, max_tokens=4096
						)
						time.sleep(SLEEP_TIME)
					except Exception as e:
				...
				if not valid_json:
					if not flag_api:
						print(
							f'文件 {filename} 经过 {max_retries} 次重试仍无法生成有效JSON，跳过处理'
						)
					continue
